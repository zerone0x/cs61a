...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for berry in branches(t):
            if berry_finder(berry):
                return True
        return False
...
def riffle(deck):
    ...
    for i in range(len(deck)//2):
        result +=[deck[:len(deck)//2][i]] + [deck[len(deck)//2:][i]]
    return result 
...
def add_trees(t1, t2):
    ...
    if len(t1) == len(t2):
        ...
        branch_t1 = branches(t1) + [tree(0) for _ in range(len(t2) - len(t1))]
        new_1 = tree(label(t1),branch_t1)
        return add_trees(new_1, t2)
    else:
